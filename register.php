...
if (isset($_POST['email']) && isset($_POST['password']) && isset($_POST['password_r']))
{
  ...
  {
    //As it's all okey, now we'll check if there is any other user with the same email
    $result = $conn->query("SELECT * FROM `users` WHERE `users_email` = '".$email."' ");
    $users = $result->fetch_all(MYSQLI_ASSOC);

    $count = count($users);

    //only if there is no other user with the same email, we proced to create the account
    if ($count == 0){
  ...
  <div class="center-block w-xxl w-auto-xs p-y-md">
    ...
    <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
      <div class="m-b text-sm">
        Sign up to your CosmosIoT Account
      </div>
